...
const SYS = 'file:///Users/timevans/projects/Vanity/vanity.mjs';
...
console.log('\n=== SEAMS: verifying the patched clone ===');
await measure("forgeHoard({post:false})", () => forge.forge.hoard({ size: 'cache', post: false }));
await measure("forgeEncounter({hoard:false,post:false})", () => forge.forge.encounter({ heat: 'fight', hoard: false, post: false }));
await measure("forgeEncounter({folderId:'delve-1',post:false,hoard:false})", () => forge.forge.encounter({ heat: 'fight', folderId: 'delve-1', post: false, hoard: false }));
await measure("forgeStage({populate:true,post:false})", () => forge.forge.stage({ type:'barrow', size:'small', populate: true, heat:'fight', activate:false, post:false, folderId:'delve-1' }));
reset();
for (const h of ['skirmish','fight','battle']) await forge.forge.stage({ type:'barrow', size:'small', populate:true, heat:h, activate:false, post:false, folderId:'delve-1' });
const t = snap();
console.log(`   3 SILENT areas → chat=${t.chat} folders=${t.folders} actors=${t.actors} scenes=${t.scenes}`);
